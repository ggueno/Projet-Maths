 ...
 * BiSparseGraphStruct.hpp created in 10 2008.
 ...
 * Anti-doublon
 */
#ifndef __OPENKN_GRAPH__BISPARSEGRAPHSTRUCT_HPP__
#define __OPENKN_GRAPH__BISPARSEGRAPHSTRUCT_HPP__
...
namespace kn{
		 ...
		 *
		 */
		class BiSparseGraphStruct : public GraphStruct{ 
		private:
		...
		private:
			...
			std::vector<unsigned int> firstPredecessor;
			std::vector<unsigned int> predecessorsList;			
		...
		public:
			...
			BiSparseGraphStruct(const GraphStruct* model);
			
			BiSparseGraphStruct(const GraphStruct& model);
			
			BiSparseGraphStruct(unsigned int size, const std::vector<std::pair<unsigned int, unsigned int> >& edges, bool directed);

			BiSparseGraphStruct* clone()const;
		public :
