 ...
 * BiSparseGraphStructTest.hpp created in 09 2008.
 ...
 * Anti-doublon
 */
#ifndef __OPENKN_GRAPH__BiSparseGRAPHSTRUCTTEST_HPP__
#define __OPENKN_GRAPH__BiSparseGRAPHSTRUCTTEST_HPP__
...
class BiSparseGraphStructTest : public CxxTest::TestSuite {
public:
	...
	void testConstructorWithEdgeList( void ) {
		...
		kn::BiSparseGraphStruct g(size, edges, true);
		...
		kn::BiSparseGraphStruct g2(size, edges, false);
  	...
  	void testNeighbours( void ){
		...
		for(int d = 0; d < 2; d++){
			kn::BiSparseGraphStruct g(size, edges, d==0);
			for(int i = 0; i < size; i++){
  	...
  	void testEdgesIterator( void ){
		...
		for(int d = 0; d < 2; d++){
			...
			bool mark[25];
	  		...
	  		kn::BiSparseGraphStruct::edges_iterator it = g.edges_begin(), end = g.edges_end();
	  		int cpt = 0;
	  		for(; it!=end; it++, cpt++){
  	...
  	void testVerticesIterator(){
  		int size = 5;
		EdgeSet edges;
		kn::BiSparseGraphStruct g(size, edges, false);
		kn::BiSparseGraphStruct::vertices_iterator it = g.vertices_begin(), end = g.vertices_end();
  		unsigned int cpt = 0;
	  	for(; it!=end; it++, cpt++){
