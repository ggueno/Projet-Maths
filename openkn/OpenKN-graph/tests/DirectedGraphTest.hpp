 ...
 * Anti-doublon
 */
#ifndef __OPENKN_GRAPH__DIRECTEDGRAPHTEST_HPP__
#define __OPENKN_GRAPH__DIRECTEDGRAPHTEST_HPP__
 ...
 * Internal Includes
 */
#include <OpenKN/graph/DirectedGraph.hpp>
/*
 * Class definition
 */
...
public:
	...
	void testConstructorWithEdgeList( void ) {
		...
		for(int i = 0; i < 4; i++){
			kn::DirectedGraph<int, int> g(size, edges, type[i]);
	...
	void testConstructorWithEdgeListAndDatas( void ) {
		...
		for(int i = 0; i < 4; i++){
			kn::DirectedGraph<int, int> g(size, vdatas, edges, edatas, type[i]);
			TS_ASSERT_EQUALS(g.size(), size);
			for(int v = 0; v < size; v++)
				TS_ASSERT(g.contains(v));
			TS_ASSERT(!g.contains(size));
	...
	void testConstructorByCopy( void ) {
		...
		for(int i = 0; i < 4; i++){
			kn::DirectedGraph<int, int> f(size, edges, type[i]);
			kn::DirectedGraph<int, int> g(f);
			...
			kn::DirectedGraph<int, int> f2(size, vdatas, edges, edatas, type[i]);
			kn::DirectedGraph<int, int> g2(f2);
 	 ...
 	 void testConstructorByCopyAndType( void ) {
		...
		for(int i = 0; i < 4; i++){
			...
			for(int i2 = 0; i2 < 4; i2++){
				kn::DirectedGraph<int, int> g(f,type[i2]);
				TS_ASSERT_EQUALS(g.size(), f.size());
				for(int v = 0; v < size+1; v++)
					TS_ASSERT_EQUALS(g.contains(v),f.contains(v));
				TS_ASSERT_EQUALS(g.isDirected(),f.isDirected());
				...
				kn::DirectedGraph<int, int> g2(f2,type[i2]);
				TS_ASSERT_EQUALS(g2.size(), f2.size());
				for(int v = 0; v < size+1; v++)
					TS_ASSERT_EQUALS(g2.contains(v),f2.contains(v));
				TS_ASSERT_EQUALS(g2.isDirected(),f2.isDirected());
  	...
  	void testIsMutable( void ){
		...
		for(int j = 0; j < 4; j++){
			kn::DirectedGraph<int, int> g(size, edges, type[j]);
  	...
  	void testEdgesIterator( void ){
		...
		for(int j = 0; j < 4; j++){
			...
			bool mark[25];
	  		...
	  		kn::DirectedGraph<int,int>::edges_iterator it = g.edges_begin(), end = g.edges_end();
	  		int cpt = 0;
	  		for(; it!=end; it++, cpt++){
  	...
  	void testVerticesIterator(){
		...
		for(int j = 0; j < 4; j++){
			kn::DirectedGraph<int, int> g(size, edges, type[j]);
			kn::DirectedGraph<int,int>::vertices_iterator it = g.vertices_begin(), end = g.vertices_end();
			unsigned int cpt = 0;
  			for(; it!=end; it++, cpt++){
  	...
  	void testSuccessors( void ){
		...
		for(int k = 0; k < 4; k++){
			kn::DirectedGraph<int, int> g(size, edges, type[k]);
			for(int i = 0; i < size; i++){
  	...
  	void testAccessTo( void ){
		...
		for(int j = 0; j < 4; j++){
			kn::DirectedGraph<int, int> g(size, edges,type[j]);
	  		for(int i = 0; i < size; i++){
				TS_ASSERT(g.accessTo(2,i));
				if(i!=2)
	  				TS_ASSERT(!g.accessTo(i,2));
	  		}
  		}
  	}
};
