 ...
 * DenseGraphStructTest.hpp created in 09 2008.
 ...
 * Anti-doublon
 */
#ifndef __OPENKN_GRAPH__DENSEGRAPHSTRUCTTEST_HPP__
#define __OPENKN_GRAPH__DENSEGRAPHSTRUCTTEST_HPP__
...
class DenseGraphStructTest : public CxxTest::TestSuite {
public:
	...
	void testConstructorWithEdgeList( void ) {
		...
		kn::DenseGraphStruct g(size, edges, true);
		...
		kn::DenseGraphStruct g2(size, edges, false);
  	...
  	void testNeighbours( void ){
		...
		for(int d = 0; d < 2; d++){
			kn::DenseGraphStruct g(size, edges, d==0);
			for(int i = 0; i < size; i++){
  	...
  	void testEdgesIterator( void ){
		...
		for(int d = 0; d < 2; d++){
			...
			bool mark[25];
	  		...
	  		kn::DenseGraphStruct::edges_iterator it = g.edges_begin(), end = g.edges_end();
	  		int cpt = 0;
	  		for(; it!=end; it++, cpt++){
  	...
  	void testVerticesIterator(){
  		int size = 5;
		EdgeSet edges;
		kn::DenseGraphStruct g(size, edges, false);
		kn::DenseGraphStruct::vertices_iterator it = g.vertices_begin(), end = g.vertices_end();
  		unsigned int cpt = 0;
	  	for(; it!=end; it++, cpt++){
