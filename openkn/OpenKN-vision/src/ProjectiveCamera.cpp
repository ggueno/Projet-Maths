...
namespace kn{
	...
	*/
	kn::Matrix<double> ProjectiveCamera::compose(const kn::Matrix3x3<double> &K,
		...
		// -RC contribution
		P.setColumn(3,-R * Vector3d(Vectord(C).getUnhomogeneous()));
	...
	*/
	...
	void ProjectiveCamera::updateRC(const kn::Matrixd &Pcanonic){
		if(Pcanonic.rows()!=3 && Pcanonic.columns()!=4)
		  throw VisionException("updateRC : Pcanonic should be a 3x4 matrix");
	...
	void ProjectiveCamera::updateK(const kn::Matrixd& K){
		...
		if(K.rows() != 3 && K.columns()!= 3)
			throw VisionException("invalid size : the input K matrix is not a 3x3 matrix");
	...
	void ProjectiveCamera::getGLModelviewMatrix(float GLMMat[16]) const{
		...
		kn::Vector<double,StaticStorePolicy<double,3> > tmp = -matR*Vector3d(Vectord(vecC).getUnhomogeneous());
	...
	*/
	...
	/*
	 * \brief compute the angle (in radians) between two rays defined by back projection the two pixels x1 and x2 (cf multiple view geometry, 2nd ed, p. 209)
	 * \param x1 : the first pixel corresponding to the first ray
	 * \param x2 : the second pixel corresponding to the second ray
     * \author Vincent
	 */
     double ProjectiveCamera::angleBetween2rays(const kn::Vector3d &x1, const kn::Vector3d &x2) const {
	    kn::Matrix3x3<double> absoluteConic = matKinverse.getTranspose() * matKinverse;
	    return acos( (x1 * absoluteConic * x2) / (sqrt(x1 * absoluteConic * x1) * sqrt(x2 * absoluteConic * x2)));
     }
     ...
     * \author Vincent
	 */
     double ProjectiveCamera::fieldOfViewX()const{
        // middle left and rignt pixels
        kn::Vector3d pix1,pix2;
        pix1[1] = pix2[1] = matK[1][2];
        pix1[2] = pix2[2] = 1.0; 
        ...
        // compute fov-x
        return angleBetween2rays(pix1,pix2);
     }
     ...
     * \author Vincent
	 */
     double ProjectiveCamera::fieldOfViewY()const{
        // middle top and bottom pixels
        kn::Vector3d pix1,pix2;
        pix1[0] = pix2[0] = matK[0][2];
        pix1[2] = pix2[2] = 1.0; 
